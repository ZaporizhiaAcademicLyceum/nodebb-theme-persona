...
<table component="groups/members" class="table table-striped table-hover" data-nextstart="{group.membersNextStart}">
	...
	<tr data-uid="{group.members.uid}">
		...
		<td class="member-name">
			<a href="{config.relative_path}/user/{group.members.userslug}"><!-- IF group.members.fullname -->{group.members.fullname}<!-- ELSE -->{group.members.username}<!-- ENDIF group.members.fullname --></a> <i title="[[groups:owner]]" class="fa fa-star text-warning <!-- IF !group.members.isOwner -->invisible<!-- ENDIF !group.members.isOwner -->"></i>
